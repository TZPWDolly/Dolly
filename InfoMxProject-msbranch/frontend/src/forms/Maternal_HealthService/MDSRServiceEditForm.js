    ...
    if (sessionStorage.getItem('editMDSR') === 'true') {
      ...
      if (pj?.status === 200 && division?.status === 200 && org?.status === 200 && death?.status === 200) {
        ...
        if (mdsrData?.status === 200) {
          ...
          setAncVisit((mData[0].MDVT).split(','))
          ...
          setSlowReason({
            first: mData[0].MDDLY1 ? true : false,
            second: mData[0].MDDLY2 ? true : false,
            third: mData[0].MDDLY3 ? true : false,
          });
